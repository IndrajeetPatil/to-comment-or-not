...
## To Comment or Not To Comment {style="margin-top: 1em; margin-bottom: 4em;"}
...
## Documentation vs. Comments {.smaller}
...
## Aside: Examples of Comment Patterns {.smaller style="background-color: #f8f9fa; padding: 15px; border-radius: 10px; border-left: 4px solid #6c757d;"}
...
|----------|-------|----------|
| **Python** | Docstrings | `"""Calculate tax based on income."""` |
| **JavaScript** | JSDoc | `/** @param {number} income - Gross income */` |
| **Java** | Javadoc | `/** Returns the calculated tax amount. */` |
| **C#** | XML Comments | `/// <summary>Calculates tax</summary>` |
| **Go** | Package comments | `// Package tax provides tax calculations` |
:::
...
|---------|---------|----------|
| **Business Rule** | `# Business rule:` | `# Business rule: VIP customers get 10% discount` |
| **Workaround** | `# HACK:` or `# WORKAROUND:` | `# HACK: API doesn't support bulk delete` |
| **Performance** | `# Performance:` | `# Performance: Cache results for 5 minutes` |
| **Security** | `# Security:` | `# Security: Validate input against XSS` |
| **TODO/FIXME** | `# TODO:` | `# TODO: Refactor to use async/await` |
:::

::: {style="margin-top: 15px; padding: 8px; background-color: rgba(108, 117, 125, 0.1); border-radius: 6px; text-align: center; font-size: 1.1em;"}
**Consistency within each project matters more than following external standards**
:::
...
```{python}
# See calculate_discount() method above
apply_discount()
```
...
```{python}
# See issue #1234 for optimization plan
...
```{python}
# Warning: complex algorithm, see docs
complex_algorithm()
```
...
# Retry connection to handle transient failures
...
## Organize Tests Structurally {.smaller}

. . .

Instead of using section comments to organize tests, use file structure or test classes.

:::: {.columns}

::: {.column width='33%'}

:::{.callout-caution icon=false title="❌ Bad: Section comments"}

```{.python filename="tests/test_calculator.py"}
# ==================
# Basic tests
# ==================
def test_addition():
...
:::{.callout-tip icon=false title="✅ Good: Separate files"}

```{.python filename="tests/test_basic_operations.py"}
def test_addition():
    ...
...
```{.python filename="tests/test_edge_cases.py"}
def test_zero_division():
    ...
...
:::{.callout-tip icon=false title="✅ Good: Test classes"}
