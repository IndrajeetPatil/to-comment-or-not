...
```{python}
# Extract domain from email by taking
...
:::{.callout-caution icon=false title="❌ Transformation unclear"}

```{python}
# Normalize price string to decimal
price = Decimal(price_str.replace(',', '').lstrip('$'))
```
...
:::{.callout-tip icon=false title="✅ Examples clarify behavior"}
