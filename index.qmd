...
**Don't be redundant by stating the obvious**
...
**Don't write vague action comments**
...
```{python}
# TODO: Switch to binary search (issue #1234)
process_data()
```
...
```{python}
# Use explicit loop instead of list comprehension
# to avoid memory issues with large datasets (10M+ items)
result = []
for item in items:
    if item > 0:
        result.append(item * 2)
...
# Binary search tried in PR #456, no benefit
for user in users:
    if user.id == target_id:
        return user
```
...
:::{.callout-caution icon=false title="❌ Outdated comment"}
...
:::{.callout-tip icon=false title="✅ Current and relevant"}

```{python}
# Use new API endpoint per v2.0 migration
refactored_solution()
```
